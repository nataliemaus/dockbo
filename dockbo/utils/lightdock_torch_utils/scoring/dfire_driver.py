...
class DFIREPotential(object):
    ...
    def __init__(self, work_dir):
        ...
        data_path = work_dir + 'dockbo/dockbo/utils/lightdock_torch_utils/scoring/data/'
        ...
        self.og_dfire_energy = self._read_potentials(data_path + "DCparams")
        self.dfire_energy = convert_energy_to_tensor(self.og_dfire_energy)
...
class DFIRE(ScoringFunction):
    ...
    def __init__(
        ...
        self.potential = DFIREPotential(work_dir=work_dir)
...
def convert_energy_to_tensor(dfire_energy):
    max_len = 20
    temp = []
    for e in dfire_energy:
        temp.append(e + [torch.nan]*(max_len - len(e)) ) 
    dfire_energy = temp
    ...
    for e in dfire_energy:
        ...
        for b in e:
            if type(b) != list:
                b = [b]
            temp_b.append(b + [torch.nan]*(max_len - len(b)) ) 
        temp.append(temp_b)
    dfire_energy = temp
    ...
    for e in dfire_energy:
        ...
        for b in e:
            ...
            for c in b:
                if type(c) != list:
                    c = [c]
                temp_c.append(c + [torch.nan]*(max_len - len(c)) ) 
        ...
        for b in e:
            ...
            for c in b:
                ...
                for f in c:
                    if type(f) != list:
                        f = [f]
                    temp_f.append(f + [torch.nan]*(max_len - len(f)) ) 
                temp_c.append(temp_f)
            temp_b.append(temp_c)
        temp.append(temp_b)
    dfire_energy = temp
    dfire_energy = torch.tensor(dfire_energy) # torch.Size([20, 14, 20, 14, 20])
